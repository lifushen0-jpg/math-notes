...
Under the hypotheses of Maschke's theorem, every finite-dimensional
representation of \(G\) is a direct sum of irreducible subrepresentations.
\end{corollary}

\begin{proof}
If \(V\) is irreducible, there is nothing to prove. Otherwise, choose a
nonzero proper \(G\)-invariant subspace \(W\subsetneq V\). By Maschke's theorem,
there is a \(G\)-invariant subspace \(U\) such that
\[
  V=W\oplus U.
\]
Both \(W\) and \(U\) have strictly smaller dimension than \(V\). Repeating the
argument and using induction on \(\dim V\), we obtain
\[
